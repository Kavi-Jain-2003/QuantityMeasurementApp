...
public class Quantity<U extends IMeasurable> {
    ...
    public boolean equals(Object obj){
        ...
        if (!((Enum<?>) this.unit).getDeclaringClass()
                .equals(((Enum<?>) other.unit).getDeclaringClass()))
            return false;


        return Math.abs(this.toBaseValue() - other.toBaseValue()) < 0.01;
    ...
    private double operations(Quantity<U> other, ArithmeticOperations opr){
        ...
        // 🔥 ADD THIS
        this.unit.validOperationSupport(opr.name());
        other.unit.validOperationSupport(opr.name());

        double base1 = this.toBaseValue();
        double base2 = other.toBaseValue();

        if(!Double.isFinite(base1) || !Double.isFinite(base2))
        ...
        switch (opr) {
            case ADD: return base1 + base2;
            case SUBTRACT: return base1 - base2;
            case DIVIDE: return base1 / base2;
            default: throw new IllegalArgumentException("Unknown Arithmetic Operation");
        }
    }
